window.addEventListener('DOMContentLoaded', () => {
	...
	chrome.tabs.query({
		...
		function updateStatus(data) {
			const tbody = contentTable.querySelector(`tbody:nth-child(${data.idx + 1})`);
			if (!tbody) {
				return;
			}
			const cell = tbody.querySelector('div.request-url');
		...
		function stringifyHeaders(headers) {
			...
			headers.forEach(item => {
				...
				text += `\n\t'${item.name.replace(/'/g, '\\\'')}': '${item.value.replace(/'/g, '\\\'')}'`;
			});
			text += '\n}';
